...
module SysTimeStampingM {
  ...
  enum {
    ...
    // 24 Jun 2005 : GWA : Cory's suggestion for TELOSB.  This is properly
    //               scaled from the 32kHz version (it's in jiffies).
    //
    // 26 Jun 2005 : GWA : Try returing to default value.
  ...
  async event void RadioReceiveCoordinator.startSymbol(uint8_t bitsPerBlock, 
                                                       ...
                                                       TOS_MsgPtr msgBuff) {
    atomic {
      rcv_time = call SysTime.getTime32();
      rcv_message = msgBuff;
    }
    return;
  }
