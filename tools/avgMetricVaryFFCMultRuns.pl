...
foreach my $currentFile (@processFiles) {
	...
	if(@fields[TTS_INDEX] != 0 && @fields[GS50_INDEX] != 0 && @fields[GS90_INDEX] != 0){
	    ...
	    #print "Got: " . $ffConstant[$lineNum] . " " . $tts[$lineNum] . " " . $gs50[$lineNum] . " " . $gs90[$lineNum] . "\n"; 
	}
	else{
	    #print "I found tts/gs values eq to 0: " . $ffConstant[$lineNum] . " " . $fields[TTS_INDEX] . " " . $fields[GS50_INDEX] . " " . $fields[GS90_INDEX] . "\n"; 
	}
	$lineNum++;
    }
...
if($metricChoice == 0){
    ...
    ($avg, $stdVals, $ffcVals, $topoTags) = &computeStats(@ttsAllRuns);
}
elsif($metricChoice == 1){
    print OUTPUT "$keysInOrder[2]\t";
    ($avg, $stdVals, $ffcVals, $topoTags) = &computeStats(@gs50AllRuns);
}
else{
    print OUTPUT "$keysInOrder[3]\t";
    ($avg, $stdVals, $ffcVals, $topoTags) = &computeStats(@gs90AllRuns);
}
...
sub computeStats {
    ...
    my @topoTags;
    my @dataArray = @_;
    #print "Doing $#{$ttsAllRuns[0]} loops of $#ttsAllRuns items \n";
