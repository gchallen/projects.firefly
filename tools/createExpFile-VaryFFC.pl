...
if ( 3 > @ARGV ) {
    print "Usage $0 numMotes time randomSeed <run Number> <topology> <topologyFile> <number of expts> <data-str>\n";
...
foreach my $ffc (@ffcVal){
    my $commandString = "$dateStr-$runNumber\t$ffc\t$topologyFile\t$numMotes\t$min\t$topology\t$randomSeed\tpacketMakefile\t250\t25\t10\n";
    print $commandString;	
}
