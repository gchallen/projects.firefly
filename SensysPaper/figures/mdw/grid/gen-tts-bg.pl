...
$TTSDIR="../../../../data/sensysData/7-Apr-GRIDS/tts_results";
$OUTFILE="tts.bars";
...
foreach $file (@files) {
  ...
  while (<FILE>) {
    if (/^(\d+)\s+(\S+)\s+(\S+)/) {
      $ff = $1;
      push @{$timetosync{$ff}}, "$nummotes $2";
      push @{$timetosync_stddev{$ff}}, "$nummotes $3";
      if ($2+$3 > $maxy) { $maxy = $2+3; }
...
print BARS "YAxis: Time to sync (sec): -labelalign center -labeldist 0.5 -grid -setmax $maxy\n";
