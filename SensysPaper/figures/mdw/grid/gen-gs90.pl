...
$TTSDIR="../../../../data/sensysData/7-Apr-GRIDS/gs90_results";
$OUTFILE="gs90.bars";
@FFLIST=(20,50,100,500);
...
foreach $file (@files) {
  ...
  while (<FILE>) {
    if (/^(\d+)\s+(\S+)\s+(\S+)/) {
      ...
      $gs90 = $2 * 1.0e6;
      $gs90_sd = $3 * 1.0e6;
      push @{$timetosync{$ff}}, "$nummotes $gs90";
      push @{$timetosync_stddev{$ff}}, "$nummotes $gs90_sd";
	...
	if ($fflist == $ff) { $inlist = 1; }
      ...
      if ($inlist) {
	if (($gs90+$gs90_sd) > $maxy) { $maxy = ($gs90+$gs90_sd); }
      }
...
print BARS "YAxis: 90th percentile group spread (usec): -labelalign center -labeldist 0.5 -grid -setmax $maxy\n";
print BARS "Key: right-(1.25*inch), top-(0.5*inch): : -keywidth 1.2 -keytitlealign right -textfont helvetica10 -fillcolor white -keyframe -elemwidth 0.1 -elemheight 0.1\n";
